...
Renders a compact, always-visible line into the `app_bottom` slot showing your API credit balance, usage, and reset period. It refreshes automatically, paints instantly from a persisted snapshot, degrades to a muted state on (networking) failure, and notifies you when credits run low.

## Features

- **Persistent `app_bottom` widget:** visible on every route; append-mode slot, so it coexists with existing footers.
...
- An OpenRouter API key. The plugin reads it from `auth.json` in OpenCodes's data dir (`~/.local/share/opencode/auth.json` on Windows: `%USERPROFILE%\.local\share\opencode\auth.json`), which OpenCodes itself writes:
...
| `lowThreshold`      | number | `10`                                    | Remaining balance (in USD) below which a low-credit notification fires.                                                                                                                                                                                      |
...
│   ├── credits.ts       # Pure helpers (tierOf, pctOf, widgetText, usd, tierColorVariant) + options parsing
│   └── credits.test.ts  # bun test unit tests
